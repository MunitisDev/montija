 ...
 *
 * **A store looks after what is inside it and nothing else.** Two attempts to make
 * an orchard's harvest survive by being *near* something were built and taken out
 * again — first doubling its yield beside a larder, then having the larder
 * preserve whatever lay within six cells of its door. Both were rules the player
 * could not see. What actually got the harvest home was hauling: a doubled load,
 * and produce that spills onto the next cell instead of evaporating when the
 * doorstep is full.
 */
